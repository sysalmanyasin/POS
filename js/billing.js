        ...
        if (inv) {
            activeCartItems.forEach(item => {
                ...
                if (prod) {
                    ...
                    if (typeof _recordInvMovement === 'function') _recordInvMovement(item.code, parseInt(item.qty, 10), 'EDIT_RESTORE', currentlyEditingInvoiceId, null, Number(prod.stock));
...
async function finalizeAndPrintBill() {
    ...
    finalDetails.forEach(item => {
        ...
        if (prod) {
            ...
            prod.stock = newStock;
            if (typeof _recordInvMovement === 'function') _recordInvMovement(item.code, -deducted, 'SALE', invoiceID, null, newStock);
...
function _doRecallLastSaved(inv) {
    ...
    activeCartItems.forEach(item => {
        ...
        if (prod) {
            prod.stock = Number(prod.stock) + parseInt(item.qty, 10);
            if (typeof _recordInvMovement === 'function') _recordInvMovement(item.code, parseInt(item.qty, 10), 'EDIT_RESTORE', inv.id, null, Number(prod.stock));
...
function processFullRefund(invoiceId) {
    ...
    (original.details || []).forEach(item => {
        ...
        if (prod) {
            if (typeof _recordInvMovement === 'function') _recordInvMovement(item.code, returnQty, 'REFUND', invoiceId, null, Number(prod.stock) + returnQty);
...
function submitPartialRefund() {
    ...
    refundLines.forEach(line => {
        ...
        if (prod) {
            if (typeof _recordInvMovement === 'function') _recordInvMovement(line.code, returnQty, 'PARTIAL_REFUND', refId, null, Number(prod.stock) + returnQty);
            prod.stock = Number(prod.stock) + returnQty;
